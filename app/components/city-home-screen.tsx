      ...
      <div id="city-story-content">
        <section className={`${styles.hero} ${styles.wrap}`} aria-labelledby="story-hero-title">
          <div className={styles.heroCopy}><p className={styles.eyebrow}>{local ? "THE LOCAL’S SIDE OF SATURDAY" : "THE NEWCOMER’S SIDE OF SATURDAY"}</p>
            ...
            <p className={styles.lede}>{local ? "You’ve got a favorite walk, a tiny cinema, a coffee counter. Share an ordinary afternoon with someone who’s just arrived. You can give a familiar place a new story." : "Moving to Minnesota? New keys. A new route home. A Saturday with nothing penciled in. Leave a little room for something besides unpacking."}</p>
            <p className={styles.explanation}>{local ? "Whiff brings four adults together for six planned activities over twelve weeks in Minneapolis and Saint Paul, Minnesota. Shared interests and a reason to keep showing up." : "Whiff brings four adults together for six planned activities over twelve weeks in Minneapolis and Saint Paul, Minnesota. Same circle, a different reason to get out the door."}</p>
            <DownloadButtons className={styles.heroDownloads} /><p className={styles.forming}>First circles forming in Minneapolis and Saint Paul.</p>
          </div>
        ...
        <SocialStory />
        <MinnesotaResources />
        <section className={`${styles.faq} ${styles.wrap}`} aria-labelledby="questions-heading"><div><p className={styles.eyebrow}>A FEW THINGS BEFORE YOU GO</p><h2 id="questions-heading">{local ? "Good question." : "Still unpacking the idea?"}</h2></div><div className={styles.questions}><details><summary>{local ? "Am I signing up to be a guide?" : "Do I have to know anyone already?"}</summary><p>{local ? "No. Whiff organizes the plans, and everyone joins in equally. You’re there for friendship through shared activities. We form circles around shared interests and availability." : "No. That’s the idea. Whiff brings a small circle together around interests and availability. Newcomers and longtime locals are welcome, and Whiff takes care of the plans."}</p></details><details><summary>Is this a dating thing?</summary><p>No. This is about platonic friendship for adults 21 and older.</p></details><details><summary>When can I start?</summary><p>Our first circles are forming in Minneapolis and Saint Paul. The App Store and Google Play links are coming soon. Downloads aren’t available here yet, and we can’t promise a start date or an immediate match.</p></details><details><summary>What does it cost?</summary><p>Membership is {PRICING.perMonth}/month after a trial of {PRICING.trialDays} days; activity costs are separate. {BET.dare} {BET.payoff} <Link href="/terms">Read the details.</Link></p></details></div></section>
      </div>

      <section className={styles.invitation} id="download" aria-labelledby="invite-heading"><span id="begin" /><div className={`${styles.invitationInner} ${styles.wrap}`}><div><p className={styles.eyebrow}>FIRST CIRCLES / MINNEAPOLIS AND SAINT PAUL</p><h2 id="invite-heading">{local ? <>A familiar place.<br />A different afternoon.</> : <>Leave a little room<br />in next Saturday.</>}</h2><p>Whiff is coming to the App Store and Google Play. Store links will appear here when they’re ready.</p></div><div className={styles.formBox}><MascotMotion videoSrc="/generated/mascot-wave.mp4" className={styles.downloadMascot} /><p className={styles.formLabel}>GET THE WHIFF APP</p><DownloadButtons /><p className={styles.formHelp}>App links coming soon. Adults 21+.<br />Membership: {PRICING.perMonth}/month after a trial of {PRICING.trialDays} days. Activities extra.</p><Link href="/terms">The details</Link></div></div></section>
    </main>
    <footer className={`${styles.footer} ${styles.wrap}`}><div><Link href="/" className={styles.wordmark}>whiff</Link><p>Go do something.<br />Then do something again.</p></div><nav aria-label="Footer"><Link href="/mn#minnesota-guides">Minnesota guides</Link><Link href="/blog">Field notes</Link><Link href="/roast">The bad stuff</Link><Link href="/states">Where we’re forming</Link><Link href="/support">Support</Link><a href={INSTAGRAM_URL} rel="me noopener">Instagram</a><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link></nav><span>© 2026 WHIFF</span></footer>
  </div>;
}

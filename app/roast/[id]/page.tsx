...
export default async function RoastDetail({ params }: Props) {
  ...
  if (!note) notFound();
  return <div className="roast-page roast-detail"><RoastHeader /><main id="main" className="roast-wrap"><div className="roast-selected"><h1>Roast Whiff.</h1><NotePaper note={note} /><nav className="roast-note-links" aria-label="Explore more"><Link href="/roast">All roasts</Link><Link href="/">Find out for yourself</Link></nav></div></main><RoastFooter /></div>;
}
